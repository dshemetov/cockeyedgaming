import re, utils, time, random, sys
import discord
import sqlite3 as sql
import config_discord as config
    ...
    if config.MUSIC_ROLE in [y.name.lower() for y in message.author.roles]:
        ...
        if match:
            ...
            if success:
                ...
                dbres = await add_to_database(message, vidid, title,
                                              message.author.name)
                if dbres:
                    response = random.sample(responses, 1)[0]
                    response = response.replace("${username}",
                               message.author.name).replace("${song}", title)
                    await client.send_message(message.channel, response)
            else:
                await client.send_message(message.channel,
                                          "Really?! :CockeyRage: (try a different YouTube link, this one couldn't download)")

async def add_to_database(message, vidid, title, added_by):
    try:
        conn=sql.connect(config.DB_LOCATION)
        cur=conn.cursor()
        cur.execute('''SELECT * FROM playlists WHERE (playlist_name = ?)''',
                    (config.DB_PLAYLIST_ID,))
        entry = cur.fetchone()
        rid = -1
        if entry is None:
            cur.execute('''INSERT INTO playlists(playlist_name, user_added) VALUES(?,?)''',
                          (config.DB_PLAYLIST_ID,"cockeyedgaming"))
            rid = cur.lastrowid
            conn.commit()
        else:
            rid = entry[0]
